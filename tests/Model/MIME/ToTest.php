 ...
 *
 * @coversDefaultClass \BLW\Model\Mime\To
 */
class ToTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->Field         = new To($this->AddressList);
    ...
    {
        # Check params
        $this->assertEquals('To', $this->Properties['Type']->getValue($this->Field), 'To::__construct() failed to set $_Type');
        $this->assertEquals(self::ADDR_LIST, $this->Properties['Value']->getValue($this->Field), 'To::__construct() failed to set $_Value');

        # Invalid Addres list
        try {
            new To(new GenericContainer);
    ...
    {
        $this->assertEquals(sprintf("To: %s\r\n", self::ADDR_LIST), @strval($this->Field), 'To::__toSting() returned an invalid format');
    }
}
