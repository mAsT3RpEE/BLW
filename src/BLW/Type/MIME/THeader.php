...
/**
 * Standard trait for MIME headers.
 *
 * <h3>RFC822</h3>
 *
 * <pre>
...
trait THeader
{
    ...
    public function parseParameter($Attribute, $Value)
    {
        ...
        if (preg_match("!^$Token+$!", $Attribute)) {
            ...
            if (preg_match("!^$Token$!", $Value)) {
                ...
                return sprintf('; %s=%s', strtolower($Attribute), $Value);
            }

            // Return $Value with quotes
            return sprintf('; %s="%s"', strtolower($Attribute), str_replace('"', '', $Value));
        }
    ...
    public function parseAddressList(IContainer $Container)
    {
        ...
        return ! ! $return ? substr($return, 2) : false;
    }
