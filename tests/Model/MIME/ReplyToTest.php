 ...
 *
 * @coversDefaultClass \BLW\Model\Mime\ReplyTo
 */
class ReplyToTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        # Check params
        $this->assertEquals('Reply-To', $this->Properties['Type']->getValue($this->Field), 'ReplyTo::__construct() failed to set $_Type');
        $this->assertEquals(self::ADDR_LIST, $this->Properties['Value']->getValue($this->Field), 'ReplyTo::__construct() failed to set $_Value');
    ...
    {
        $this->assertEquals(sprintf("Reply-To: %s\r\n", self::ADDR_LIST), @strval($this->Field), 'ReplyTo::__toSting() returned an invalid format');
    }
}
