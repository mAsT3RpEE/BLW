<?php
/**
 * ContentBaseTest.php | Mar 10, 2014
 *
 * @filesource
 * @license MIT
 ...
 *
 * @coversDefaultClass \BLW\Model\Mime\ContentBase
 */
class ContentBaseTest extends \PHPUnit_Framework_TestCase
{
    /**
    ...
    {
        # Check params
        $this->assertEquals('Content-Base', $this->Properties['Type']->getValue($this->Header), 'ContentBase::__construct() failed to set $_Type');
        ...
        foreach($this->generateValidBases() as $Parameters) {
            ...
            $this->assertEquals($Expected, $this->Properties['Value']->getValue($this->Header), sprintf('ContentBase::__contruct(%s) failed to set $_Value', $Input));
        }
    ...
    {
        $this->assertEquals("Content-Base: ftp://example.com\r\n", @strval($this->Header), 'ContentBase::__toSting() returned an invalid format');
    }
}
