 ...
 *
 * @coversDefaultClass \BLW\Model\Mime\BCC
 */
class BCCTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->Field         = new BCC($this->AddressList);
    ...
    {
        # Check params
        $this->assertEquals('Bcc', $this->Properties['Type']->getValue($this->Field), 'BCC::__construct() failed to set $_Type');
        $this->assertEquals(self::ADDR_LIST, $this->Properties['Value']->getValue($this->Field), 'BCC::__construct() failed to set $_Value');

        # Invalid Addres list
        try {
            new BCC(new GenericContainer);
    ...
    {
        $this->assertEquals(sprintf("Bcc: %s\r\n", self::ADDR_LIST), @strval($this->Field), 'BCC::__toSting() returned an invalid format');
    }
}
